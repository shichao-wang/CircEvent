...
def get_arguments(argv):
    ...
    parser.add_argument('-r', '--sample_rel', action='store_true',
                        default=False,
...
def main():
    ...
    if args.prev_folder:
        fpath = os.path.join(args.prev_folder,
                             'argw_enc_{}.pt'.format(args.epoch_batch))
    ...
    if args.prev_folder:
        fpath = os.path.join(args.prev_folder,
                             'model_{}.pt'.format(args.epoch_batch))
        logging.info('loading {}...'.format(fpath))
        model.load_state_dict(
            torch.load(fpath, map_location=lambda storage, location: storage))

    sampler = NegativeSampler([config['arg0_max_len'], config['arg1_max_len'],
                               config['arg2_max_len']],
                              config['n_rel_types'],
    ...
    if args.prev_folder:
        fpath = os.path.join(args.prev_folder,
                             'optim_{}.pt'.format(args.epoch_batch))
        logging.info('loading {}...'.format(fpath))
    ...
    for i_epoch in range(config['n_epochs']):
        ...
        for i_file, f in enumerate(files):
            ...
            for i_batch, (x, y) in enumerate(train_loader):
                ...
                # sampling
                x_neg = Variable(sampler.sampling(x).to(args.device),
                                 requires_grad=False)
                ...
                for i in range(1, neg_ratio):
                    n_score[:n_samples] = n_score[:n_samples] + n_score[
                                                                i * n_samples: (
                                                                                           i + 1) * n_samples]
                ...
                if args.normalize_embeddings:
                    model.rel_embeddings.weight.data = F.normalize(
                        model.rel_embeddings.weight.data,
                        p=model.norm, dim=1)
                    model.pred_embeddings.weight.data = F.normalize(
                        model.pred_embeddings.weight.data,
                        p=model.norm, dim=1)
                ...
                if min_loss:
                    if tmp_loss < min_loss:
                        ...
                        min_loss = tmp_loss
                        best_epoch = i_epoch
                        save(args.output_folder, model, optimizer, i_epoch,
                             i_file, i_batch)
                    ...
                    min_loss = tmp_loss
                    best_epoch = i_epoch
                    save(args.output_folder, model, optimizer, i_epoch, i_file,
                         i_batch)
                    save_losses(args.output_folder, losses)

                if i_batch % config['n_batches_per_record'] == 0:
                    logging.info(
                        "{}, {}, {}: loss={}, time={}".format(i_epoch, i_file,
                                                              i_batch, tmp_loss,
                                                              time.time() - epoch_start))
                    save_losses(args.output_folder, losses)

        logging.info("epoch {}: loss={}, time={}".format(i_epoch, tmp_loss,
                                                         time.time() - epoch_start))
